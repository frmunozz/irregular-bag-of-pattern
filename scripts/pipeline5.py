...
def grid_search(data, labels, wins, wls, alphabet_size, q, classes, lsa_kwargs, doc_kwargs, out_path, spatial_comp=100):
    ...
    for q_i in q:
        for q_ii in q_i:
            f_name += symbols[q_ii]
        f_name += "-"
    f_name += "alph_size_" + str(alphabet_size) + "-"
...
if __name__ == '__main__':
    ...
    # out path
    out_path = os.path.join("..", "data", "configs_results", "alphabet_size_search")

    for alph in alphabet_size:
        grid_search(dataset, labels_, wins, wls, alph, q, classes, lsa_kwargs, doc_kwargs, out_path,
                    spatial_comp=sc)
